...
conkylogfile="/etc/esubuntu/log_esubuntu.log"
touch $conkylogfile
...
if ! grep "%professeurs ALL=(ALL) ALL" /etc/sudoers > /dev/null; then
	...
	sed -i "/%admin ALL=(ALL) ALL/a\%professeurs ALL=(ALL) ALL" /etc/sudoers 2>> $conkylogfile
	sed -i "/%admin ALL=(ALL) ALL/a\%DomainAdmins ALL=(ALL) ALL" /etc/sudoers 2>> $conkylogfile
fi
...
if ! grep "esubuntuapt" /etc/apt/apt.conf.d/20proxy > /dev/null; then
	echo "--- Mise à jour nécessaire"
	sed -i 's/apt.esubuntu/esubuntuapt/g' /etc/apt/apt.conf.d/20proxy 2>> $conkylogfile
fi
if grep "https://" /etc/apt/apt.conf.d/20proxy > /dev/null; then
	sed -i 's/https:\//http:\//g' /etc/apt/apt.conf.d/20proxy 2>> $conkylogfile
fi
...
if ! grep "esubuntuapt" /etc/wgetrc > /dev/null; then
	echo "--- Mise à jour nécessaire"
	sed -i 's/apt.esubuntu/esubuntuapt/g' /etc/wgetrc 2>> $conkylogfile
fi
if ! grep "esubuntuapt" /etc/environment > /dev/null; then
	sed -i 's/172.18.248.1/esubuntuapt:Zaf1r4poRSrt4dkkfs2d12z5@172.18.248.1/g' /etc/environment 2>> $conkylogfile
fi
