...
wget -nc -q --no-check-certificate http://download.tuxfamily.org/polyglotte/archives/msfonts-config2.zip -P ./tmp_dl/
...
if $WPSOffice; then
	...
	# Logiciel
	wget -nc -q --no-check-certificate http://wdl1.pcfg.cache.wpscdn.com/wpsdl/wpsoffice/download/linux/$build/wps-office_$version.XA_amd64.deb
	dpkg -i wps-office_$version.XA_amd64.deb
	# Dictionnaire FR
	apt install p7zip p7zip-full hunspell-fr -y
	wget -nc -q --no-check-certificate http://grammalecte.net/download/fr/hunspell-french-dictionaries-v7.0.zip
...
if [ ! -e ./tmp_dl/TexMaths-0-v2.49.oxt ]; then
	wget -nc -q https://extensions.libreoffice.org/assets/downloads/1236/TexMaths-0-v2.49.oxt -P ./tmp_dl/
fi
...
wget -nc -q --no-check-certificate http://cdeval.free.fr/IMG/ttf/Cmath.ttf -P /usr/share/fonts
wget -nc -q --no-check-certificate http://cdeval.free.fr/IMG/ttf/cmathscr.ttf -P /usr/share/fonts
wget -nc -q --no-check-certificate http://cdeval.free.fr/IMG/ttf/cmathcal.ttf -P /usr/share/fonts
