...
function autoparam {
  find $1/dans_icones/groupe_esu/ -type f -exec sed -i -e "s/RNE_ETAB/"$rne_etab"/g" -e "s/IP_SCRIBE/"$scribe_def_ip"/g" {} \;
  find $1/dans_icones/groupe_esu/ -type f -exec sed -i -e "s/IP_PRONOTE/"$pronote"/g" -e "s/PORTAIL/"$portail"/g" {} \;
  find $1/dans_icones/groupe_esu/ -type f -exec sed -i -e "s/SALLEESU/"$salle"/g" -e "s/PROXY_IP/"$proxy_def_ip"/g" {} \;
  find $1/dans_icones/groupe_esu/ -type f -exec sed -i -e "s/PROXY_PORT/"$proxy_def_port"/g" -e "s/GSETPROXYPORT/"$gset_proxy_port"/g" {} \;
  find $1/dans_icones/groupe_esu/ -type f -exec sed -i -e "s/GSETPROXY/"$gset_proxy"/g" -e "s/SUBNET/"$subnet"/g" {} \;
  find $1/dans_icones/groupe_esu/ -type f -exec sed -i -e "s/INTERFACEETH/"$interfaceeth"/g" {} \;
}
