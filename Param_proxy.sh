...
port=$proxy_def_port" > /usr/share/glib-2.0/schemas/my-defaults.gschema.override 2>> $logfile
	...
	addtoend /etc/environment "HTTP_PROXY=http://$proxy_def_ip:$proxy_def_port/" "HTTPS_PROXY=http://$proxy_def_ip:$proxy_def_port/" "http_proxy=http://$proxy_def_ip:$proxy_def_port/" "https_proxy=http://$proxy_def_ip:$proxy_def_port/" "ftp_proxy=ftp://$proxy_def_ip:$proxy_def_port/" "no_proxy=\"$proxy_env_noproxy\"" 2>> $logfile
	
	#Paramétrage du Proxy pour apt
	######################################################################
	writelog "---Inscription du proxy pour apt"
	echo "APT::Get::AllowUnauthenticated 1;
Acquire::http::proxy \"http://$scribeuserapt:$scribepass@$proxy_def_ip:$proxy_def_port/\";
Acquire::ftp::proxy \"ftp://$scribeuserapt:$scribepass@$proxy_def_ip:$proxy_def_port/\";
Acquire::https::proxy \"http://$scribeuserapt:$scribepass@$proxy_def_ip:$proxy_def_port/\";" > /etc/apt/apt.conf.d/20proxy 2>>$logfile
